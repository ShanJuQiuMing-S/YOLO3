...
def yolo_head(feats, anchors, num_classes, input_shape, calc_loss=False):
    ''' 根据特征层输出的结果和先验框 计算预测框、置信度和概率 '''
    num_anchors = len(anchors)
    # [1, 1, 1, num_anchors, 2]
    # 1 先验框的宽高
    anchors_tensor = K.reshape(K.constant(anchors), [1, 1, 1, num_anchors, 2])

    # 2 获得x，y的网格,先验框的中心点坐标 (13, 13, 1, 2)
    ...
    # 设置featuremap的shape (batch_size,13,13,3,25), 3=anchors, 25=20+4+1
...
def yolo_loss(args, anchors, num_classes, ignore_thresh=.5, print_loss=False):
    ...
    for l in range(num_layers):
        ...
        object_mask = y_true[l][..., 4:5]  # 4+1+20 置信度
        ...
        grid, raw_pred, pred_xy, pred_wh = yolo_head(yolo_outputs[l],
             anchors[anchor_mask[l]], num_classes, input_shape, calc_loss=True)  # 网格坐标、特征、预测框坐标
        
        # 这个是解码后的预测的box的位置，(m,13,13,3,4)
